...
public class TreeTestWalker<T> {
	...
	public TreeTestWalker(Class<? extends IUrlStartPointConverter<T>> urlSpc,
			PageWalker<T> pageWalker, Class<? extends ITransitionConverter<T>> tc,
	 ...
	 * If targetExtensionPoint is non-null, only converts test on path to that ExtensionPoint.
	 * @param test
	 * @param t
	 * @param targetExtensionPoint if non-null, only convert test on path to this ExtensionPoint.
	 */
	...
	/**
	 * Converts test node and its children.
	 * If targetExtensionPoint is non-null, only converts test on path to that ExtensionPoint.
	 * If node is subtest, only convert the used path in the subtest 
	 * (the path leading to the extension point that is extended from)
	 * 
	 * @param t
	 * @param node
	 * @param targetExtensionPoint if non-null, only convert node on path to this ExtensionPoint.
	 */
	private void convertTransitionNode(T t, TransitionNode node, ConnectionPoint targetExtensionPoint)
			throws InstantiationException, IllegalAccessException {

		if (targetExtensionPoint == null || isOnExtensionPointPath(node, targetExtensionPoint)) {

			if(node instanceof UrlStartPoint){
				urlStartPointConverter.newInstance().handleUrlStartPoint(t,	(UrlStartPoint) node);
			} else if (node instanceof ExtensionStartPoint) {
				convertTest(((SubTest) node).getTest(), t, (ExtensionStartPoint) node);
			} else if (node instanceof SubTest) {
				Test test = ((SubTest) node).getTest();
				List<Transition> out = node.getOutTransitions();
				if (out.size() > 1) {
					throw new CubicException("More than one out transition from subtest is not allowed.");
				} else if (out.size() == 1) {
					//only convert path in subtest leading to the extension point that is extended from
					convertTest(test, t, ((ExtensionTransition) out.get(0)).getExtensionPoint());
				} else {
				...
				customTestStepConverter.newInstance().handleCustomStep(t, (CustomTestStep) node);
			} else {
				return;
			}
			...
			for (Transition outTransition : node.getOutTransitions()) {
				...
				if (targetExtensionPoint == null || isOnExtensionPointPath(endNode, targetExtensionPoint)) {
					transitionConverter.newInstance().handleTransition(t, outTransition);
					//recursive:
		...
		else if (targetExtensionPoint != null && (node instanceof UrlStartPoint || node instanceof ExtensionStartPoint)) {
			//start point was not on path to extension point. Not possible to continue.
			//show decent error message:
			String pageId = "unknownId";
			if (targetExtensionPoint instanceof ExtensionStartPoint)
				pageId = ((ExtensionStartPoint) targetExtensionPoint).getSourceExtensionPointPageId();
			else if (targetExtensionPoint instanceof ExtensionPoint)
				pageId = ((ExtensionPoint) targetExtensionPoint).getPageId();
			throw new UnknownExtensionPointException("Target extension point connected to page with ID " +
	 ...
	 */
	public boolean isOnExtensionPointPath(TransitionNode node, ConnectionPoint targetExtensionPoint) throws InstantiationException, IllegalAccessException {
		...
		if (targetExtensionPoint instanceof ExtensionStartPoint) {
			...
			throw new CubicException("Unsupported connection point: " + targetExtensionPoint.toString());
		}
