...
public class TransitionConverter implements ITransitionConverter<SeleneseDocument> {
	 ...
	 */
	public void handleUserInteractions(SeleneseDocument steps, UserInteractionsTransition userInteractions) {
	 ...
	 */
	private void handlePageElementAction(SeleneseDocument steps, UserInteraction peAction) {
		...
		if (obj instanceof PageElement) {
			...
			if (element.getIdentifierType().equals(LABEL) && element instanceof FormElement && 
				...
				SeleniumUtils.appendGetLabelTargetId(buff, element, element.getDescription());
				idText = "\" + labelTargetId + \"";
				idType = ":id";
				steps.add(new Command(buff.toString()).setDescription("Getting label with text = '" + element.getText()));
			}

			if(action.equals(CLICK)){
				step = new Command("ie." + elementType + "(" + idType + ", \"" + idText + "\").click");
				...
				if (parent.getIdentifierType().equals(IdentifierType.LABEL)) {
					//If label, inject script to get the ID from the label and modify variables with the injected value:
					StringBuffer buff = new StringBuffer();
					SeleniumUtils.appendGetLabelTargetId(buff, parent, parent.getDescription());
					parentIdText = "\" + labelTargetId + \"";
					parentIdType = ":id";
					steps.add(new Command(buff.toString()).setDescription("Getting select list with text = '" + parent.getText()));
				}
				String selectList = "ie.select_list(" + parentIdType + ", \"" + parentIdText + "\")";
				
				if (element.getIdentifierType().equals(LABEL)) {
					step = new Command(selectList + ".select(\"" + idText + "\")");
				}
				else {
					step = new Command(selectList + ".option(" + idType + ", \"" + idText + "\").select");
				}
				...
				if (element instanceof Select) {
					step = new Command("ie." + elementType + "(" + idType + ", \"" + idText + "\").option(:value, \"" + input +"\").select");
					step.setDescription("Selecting option with value '" + input + "' in " + element.getType() + " with " + idType + " = '" + idText + "'");
				}
				else {
					step = new Command("ie." + elementType + "(" + idType + ", \"" + idText + "\").set(\"" + input +"\")");
				...
				String eventType = SeleniumUtils.getEventType(action);
				step = new Command("ie." + elementType + "(" + idType + ", \"" + idText + "\").fireEvent(\"" + eventType + "\")");
