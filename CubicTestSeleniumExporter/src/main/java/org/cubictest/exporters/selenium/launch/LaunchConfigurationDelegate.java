...
import org.cubictest.exporters.selenium.runner.holders.RemoteSeleniumHolder;
...
public class LaunchConfigurationDelegate extends AbstractJavaLaunchConfigurationDelegate {
	...
	public void launch(ILaunchConfiguration configuration, String mode,
		...
		try {
			...
			port = evaluatePort();
			launch.setAttribute(CUBIC_UNIT_PORT, String.valueOf(port));
			...
			IFile testFile = project.getProject().getFile("tests/log_in.aat");
			...
			RemoteSeleniumHolder holder = new RemoteSeleniumHolder(
					new CubicTestProjectSettings(
							project.getProject()));
			
			try {
				holder.setSelenium(new CubicTestRemoteRunnerClient(port));
			} catch (Exception e) {
	...
	protected void preLaunchCheck(ILaunchConfiguration configuration, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		try {
			...
			if( javaProject != null && javaProject.getProject().getFile(new Path("tests/log_in.aat")) != null){
				return;
			}
			ErrorHandler.logAndShowErrorDialog(REMOTE_CUBIC_RUNNER_IS_NOT_ON_THE_CLASSPATH);
			throw new CoreException(new Status(IStatus.ERROR, 
					SeleniumExporterPlugin.PLUGIN_ID,
	...
	protected void collectExecutionArguments(ILaunchConfiguration configuration, List/*String*/ vmArguments, List/*String*/ programArguments) throws CoreException {
		...
		programArguments.add("-port:" + String.valueOf(port)); //$NON-NLS-1$
	}
}
